...
public:
     ...
     */
    BLEProcess(events::EventQueue &event_queue, BLE &ble_interface, std::string n) :
    ...
    void start()
    {
        //abmt::os::log("Ble process started.\r\n");

        if (_ble.hasInitialized()) {
            //abmt::os::log("Error: the ble instance has already been initialized.\r\n");
            return;
        }
        ...
        if (error) {
            //abmt::os::log_err("Error returned by BLE::init.\r\n");
            return;
        }
    ...
    void stop()
    {
        if (_ble.hasInitialized()) {
            _ble.shutdown();
            //abmt::os::log("Ble process stopped.");
        }
    }
...
protected:
    ...
    {
        if (event->error) {
            //abmt::os::log_err("Error during the initialisation\r\n");
            return;
        }

        //abmt::os::log("Ble instance initialized\r\n");
    ...
    void start_advertising()
    {
        ...
        if (error) {
            //abmt::os::log("_ble.gap().setAdvertisingParameters() failed\r\n");
            return;
        }
        ...
        if (error) {
            //abmt::os::log_err("Gap::setAdvertisingPayload() failed\r\n");
            return;
        }
        ...
        if (error) {
            //abmt::os::log_err("Gap::startAdvertising() failed\r\n");
            return;
        }
